...
Sphere.prototype = {
  ...
  raycast: function(r1) {
    ...
    var point_x1 = ((-1*b +Math.sqrt(discriminant))/(2*a));
    var point_x2 = ((-1*b-Math.sqrt(discriminant))/(2*a));
    ...
    if(point_x1 < point_x2){
      var result ={
        hit : true,
        point: r1.direction.clone().multiplyScalar(point_x1).add(r1.origin),
        normal:r1.direction.clone().multiplyScalar(point_x1).add(r1.origin).subtract(this.center).normalize(),
        distance: distance
      }
    }
    if (point_x1 > point_x2){
      var result ={
        hit : true,
        point: r1.direction.clone().multiplyScalar(point_x2).add(r1.origin),
        normal:r1.direction.clone().multiplyScalar(point_x2).add(r1.origin).subtract(this.center).normalize(),
        distance: distance
      }
    }
